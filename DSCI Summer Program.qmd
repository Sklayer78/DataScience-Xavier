...
# length(unique(data2$`Student ID-Random`)) - length(unique(data1$`Student ID-Random`))
...
ggplot(data = sample1, aes(x = Class_Count))+
  ...
  labs(x = "Number of Attempted Language Courses", y = "Count of Students")+
...
data7 <- Data %>% 
  ...
  mutate(Retention = 0)
# Step 3: Combine 2 datasets
combined_data7 <- data7 %>%
  left_join(data2, by = "Student ID-Random") %>%
  mutate(Retention = coalesce(Retention_Data, Retention)) %>%  # Override Retention with values from data2
  select(-Retention_Data) 

sample7 <- combined_data7 %>%
  filter(Retention == "1") %>%
  pivot_longer(cols = 12:33, names_to = "Credit", values_to = "Taken") %>%
  filter(Taken == "1") %>%
  group_by(`Student ID-Random`, Credit) %>%
...
ggplot(data = sample7, aes(x = Class_Count))+
